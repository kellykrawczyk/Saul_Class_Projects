...
public class SVM {
    ...
    public void testDay2() {
        ...
        try {
            ...
            for (int j = 0; j < DataReader.SYMBOLS.length; j++) {

                loadData(j);

                int k = historicalData.size() - numberOfExample - featureLength + 1;

                PrintWriter f1 = new PrintWriter(new FileWriter("SVM/libsvm-3.20/stock/" + "SVMHistroy_" + DataReader.SYMBOLS[j] + "_test" + k + ".txt"));
                ...
                try {
                    ...
                    BufferedReader bufferedReader = new BufferedReader(new FileReader("SVM/libsvm-3.20/stock/" + "SVMHistroy_" + DataReader.SYMBOLS[j] + "_result" + i + ".txt"));
                    String line;
        ...
        try {
            ...
            for (int j = 0; j < DataReader.SYMBOLS.length; j++) {
                ...
                for (int k = 0; k < historicalData.size() - numberOfExample - featureLength; k++) {
                    ...
                    PrintWriter f0 = new PrintWriter(new FileWriter("SVM/libsvm-3.20/stock/" + "SVMHistroy_" + DataReader.SYMBOLS[j] + "_train" + k + ".txt"));
                    PrintWriter f1 = new PrintWriter(new FileWriter("SVM/libsvm-3.20/stock/" + "SVMHistroy_" + DataReader.SYMBOLS[j] + "_test" + k + ".txt"));
            ...
            for (int j = 0; j < DataReader.SYMBOLS.length; j++) {
                ...
                for (int i = 0; i < historicalData.size() - numberOfExample - featureLength; i++) {
                    try {
                        ...
                        while ((line = bufferedReader.readLine()) != null) {
                            ...
                            statement.executeUpdate("INSERT INTO PredictionSVM VALUES ('" + DataReader.SYMBOLS[j] + "', '" + historicalDate.get(m) + "', " + Math.round(Double.parseDouble(line) * 100.0) / 100.0 + "," + Math.abs(Math.round(difference * 100.0) / 100.0) + ", '" + action + "')");
                            m++;
                        }
                ...
                try {
                    ...
                    while ((line = bufferedReader.readLine()) != null) {
                        ...
                        statement.executeUpdate("INSERT INTO PredictionSVM VALUES ('" + DataReader.SYMBOLS[j] + "', 'predict day 1', " + Math.round(Double.parseDouble(line) * 100.0) / 100.0 + ", 0, '" + action + "')");
                ...
                try {
                    ...
                    while ((line = bufferedReader.readLine()) != null) {
                        ...
                        statement.executeUpdate("INSERT INTO PredictionSVM VALUES ('" + DataReader.SYMBOLS[j] + "', 'predict day 2', " + Math.round(Double.parseDouble(line) * 100.0) / 100.0 + ", 0, 'NULL')");
    ...
    public void testWithRDP() {
        ...
        try {
            ...
            for (int j = 0; j < DataReader.SYMBOLS.length; j++) {
                ...
                PrintWriter f = new PrintWriter(new FileWriter("SVM/libsvm-3.20/stock/" + "SVMHistroy_" + DataReader.SYMBOLS[j] + "_test.txt"));
    ...
    private void loadData(int CompanyIndex) {
        ...
        try {
            ...
            ResultSet res = statement.executeQuery("SELECT * FROM StockHistorical WHERE Symbol = '" + DataReader.SYMBOLS[CompanyIndex] + "'");
