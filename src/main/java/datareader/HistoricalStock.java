...
public class HistoricalStock extends TimerTask {
    ...
    public HistoricalStock(int y) {
        numYear = y;
        listHistory = (List<HistoricalQuote>[]) new List[DataReader.SYMBOLS.length];

        Calendar from = Calendar.getInstance();
        from.add(Calendar.YEAR, -numYear);

        for (int i = 0; i < DataReader.SYMBOLS.length; i++) {
            Stock stock = YahooFinance.get(DataReader.SYMBOLS[i], from, Interval.DAILY);
    ...
    public void collectData() {
        ...
        try {
            ...
            for (int i = 0; i < DataReader.SYMBOLS.length; i++) {
                ...
                String symbol = DataReader.SYMBOLS[i];
                String date;
